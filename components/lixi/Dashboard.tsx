    ...
    <section id="dashboard" className="bg-[#fff7ed] py-20 scroll-mt-27.5">
      <div className="mx-auto w-[min(1180px,94vw)]">
        <div className="mb-8 text-center">
          <h2 className="mb-2.5 text-3xl font-bold sm:text-4xl">Bảng tóm tắt nhanh</h2>
          <p className="text-[#6a5c55]">
            Những con số minh họa để bạn yên tâm rút ví (và bật cười).
          </p>
        </div>

        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {kpiData.map((kpi, index) => (
            <AnimatedView key={kpi.label} delay={index * 0.06}>
              <article className="min-h-40.5 relative overflow-hidden rounded-[18px] border border-black/6 bg-white/86 p-4.5 shadow-[0_24px_60px_rgba(215,38,61,0.12)] backdrop-blur-md">
                <div
                  ...
                  aria-hidden="true"
                />
                <p className="m-0 font-bold text-[#6a5c55]">{kpi.label}</p>
                <p
                  className={cn(
                    'my-1.5 mb-2.5 text-3xl font-extrabold',
                    kpi.tone && toneClass[kpi.tone]
                  )}
                >
                  <Counter target={kpi.target} format={kpi.format} suffix={kpi.suffix} />
                </p>
                <p className="m-0 text-[#6a5c55]">{kpi.note}</p>
              </article>
            </AnimatedView>
          ))}
          ...
          <AnimatedView delay={kpiData.length * 0.06}>
            <article className="relative overflow-hidden rounded-[18px] border-t-4 border-t-[#d7263d] border-black/6 bg-white/86 p-4.5 shadow-[0_24px_60px_rgba(215,38,61,0.12)] backdrop-blur-md">
              <p className="m-0 font-bold text-[#6a5c55]">Mục tiêu tháng này</p>
              <p className="my-1.5 mb-2.5 text-3xl font-extrabold">10.000.000 đ</p>
              <div className="h-2.5 overflow-hidden rounded-full bg-black/5" aria-label="35% đạt">
                <span
                  className="block h-full rounded-full"
                  style={{ width: '35%', background: 'linear-gradient(90deg, #d7263d, #f28c28)' }}
                />
              </div>
              <p className="m-0 mt-2.5 text-[#6a5c55]">Đã đạt 35% nhờ mọi người</p>
            </article>
          </AnimatedView>
        </div>
      </div>
