  ...
  return (
    <section id="commit" className="bg-[#fff7ed] py-20 scroll-mt-27.5">
      <div className="mx-auto w-[min(1180px,94vw)]">
        ...
        >
          <h2 className="mb-2.5">Cam kết vàng</h2>
          <p className="mb-3 text-[#6a5c55]">Viết bằng giọng vui nhưng thực hiện nghiêm túc, đúng vibe Tết.</p>
          <ul className="grid list-none gap-2.5 p-0 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
            {commitData.map((item) => (
              <li key={item.title} className="rounded-[14px] border border-dashed border-black/7 bg-white/90 p-3">
