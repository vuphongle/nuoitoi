  ...
  for (const { locale, source } of localeFiles) {
    for (const [section, keys] of Object.entries(requiredSections)) {
      ...
      for (const key of keys) {
        assert.match(source, new RegExp(`\\b${key}:\\s*['\\"]`), `${locale} is missing ${section}.${key}`);
      }
    }
  }
});
...
test('login translations use the Card Platform brand', () => {
  for (const { locale, source } of localeFiles) {
    ...
    assert.match(source, /bannerTitleLine1:\s*['"]Card Platform['"]/, `${locale} uses the wrong login brand`);
  }
