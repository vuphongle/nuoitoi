...
export function TopNavigation({ viewer, demoMode }: { viewer: Employee; demoMode: boolean }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-[rgba(255,250,246,0.88)] backdrop-blur-xl">
        <div className="app-container flex h-16 items-center justify-between gap-4">
          <Link className="flex items-center gap-3" href="/" aria-label="Nhà Mình - Trang chủ">
            ...
            <span>
              <strong className="block text-sm font-black tracking-[-0.02em] text-slate-950">NHÀ MÌNH</strong>
          ...
          <nav aria-label="Điều hướng chính" className="hidden items-center gap-1 rounded-2xl border border-slate-200/70 bg-white/70 p-1 sm:flex">
            {navItems.map((item) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <Link className="flex items-center gap-2 rounded-full p-1 transition hover:bg-white" href="/profile/edit" aria-label={`Mở hồ sơ của ${viewer.fullName}`}>
              <span className="hidden text-right lg:block">
                <strong className="block max-w-36 truncate text-xs text-slate-900">{viewer.fullName}</strong>
      ...
      <nav aria-label="Điều hướng trên điện thoại" className="fixed inset-x-3 bottom-3 z-50 grid grid-cols-3 rounded-2xl border border-white/80 bg-slate-950/95 p-1.5 shadow-2xl backdrop-blur sm:hidden">
        {navItems.map((item) => {
          const active = isActive(pathname, item.href);
          return (
            <Link
