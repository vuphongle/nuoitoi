...
export default async function EmployeeDetailPage({ params }: PageProps<'/employees/[id]'>) {
  ...
  return (
    <AppShell viewer={viewer}>
      <Link
        className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-coral-700"
        href="/employees"
      >
      ...
      <section className="surface-card overflow-hidden">
        <div className="relative h-36 bg-gradient-to-r from-coral-100 via-orange-50 to-mint-100 sm:h-44">
          <span className="absolute right-8 top-5 text-5xl text-white/70" aria-hidden>
            ✦
          </span>
        </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {employee.interests.length > 0 ? (
                    employee.interests.map((interest) => (
                      <span
                        key={interest}
                        className="rounded-full border border-coral-100 bg-white px-3 py-1.5 text-xs font-bold text-coral-700"
                      >
                        {interest}
                      </span>
                    ))
