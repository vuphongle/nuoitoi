    ...
    <main className="grid min-h-screen lg:grid-cols-[1.1fr_0.9fr]">
      <section className="relative hidden overflow-hidden bg-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-2xl bg-white text-sm font-black text-slate-950">
            NM
          </span>
          <strong className="text-lg font-black tracking-tight">NHÀ MÌNH</strong>
        </div>
        <div className="relative max-w-xl">
          <span className="text-5xl text-coral-300" aria-hidden>
            “
          </span>
        ...
        <p className="relative text-xs font-semibold text-slate-500">
          Chỉ dành cho thành viên công ty.
        </p>
      </section>
      ...
      <section className="grid place-items-center px-5 py-10 sm:px-10">
        <div className="w-full max-w-md">
          <div className="mb-8 flex items-center gap-3 lg:hidden">
            <span className="grid size-10 place-items-center rounded-xl bg-slate-950 text-xs font-black text-white">
              NM
            </span>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-[-0.04em] text-slate-950">
            Đăng nhập vào ngôi nhà chung
          </h2>
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Sử dụng email công ty để tiếp tục.
          </p>

          {errorCode && messages[errorCode] && (
            <p
              className="mt-5 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-800"
              role="alert"
            >
              {messages[errorCode]}
            </p>
